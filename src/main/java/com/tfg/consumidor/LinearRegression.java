...
public class LinearRegression {
    ...
    private static Logger logger = LogManager.getLogger(LinearRegression.class);

   /**
     * Performs a linear regression on the data points {@code (y[i], x[i])}.
     *
     * @param  tiempos the values of the predictor variable
     * @param  y the corresponding values of the response variable
     * @throws IllegalArgumentException if the lengths of the two arrays are not equal
     */
    public LinearRegression(ArrayList<Double> tiempos,ArrayList <Double> y) {
        ...
        for (int i = 0; i < n; i++) {
            sumx  += tiempos.get(i);
            sumx2 += tiempos.get(i)*tiempos.get(i);
            sumy  += y.get(i);
        }
        ...
        for (int i = 0; i < n; i++) {
            xxbar += (tiempos.get(i) - xbar) * (tiempos.get(i) - xbar);
            yybar += (y.get(i) - ybar) * (y.get(i) - ybar);
            xybar += (tiempos.get(i) - xbar) * (y.get(i) - ybar);
        }
        slope  = xybar / xxbar;
        ...
        for (int i = 0; i < n; i++) {
            double fit = slope*tiempos.get(i) + intercept;
